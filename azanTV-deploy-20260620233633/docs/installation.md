...
1. Open the Tizen Studio workspace that contains the **`azanTV`** project (or **File → Open Projects from File System** and select the `azanTV` folder)
2. Confirm **API Version 4.0** and profile **tv-samsung**
3. Verify `config.xml` application id: `GfnCKw2I8W.AzanTV`

The buildable Tizen Studio project lives in the **`azanTV/`** folder. The repository root also contains the same application sources used during development.

Or create an empty TV 4.0 project named `azanTV` and copy the application files into it (preserve `.project`, `.settings`, `.tproject`).

## 6. Build and install

**Real TV (German step-by-step):** see [installation-real-tv-de.md](installation-real-tv-de.md)

**One-command deploy (Windows, TV connected):**
...
- DST is handled by the TV local clock; adhan-js returns local `Date` values used directly
...
```bash
tizen uninstall -p GfnCKw2I8W.AzanTV -t YOUR_TV_NAME
```
